...
- マークダウン記法
  ...
  - リンク：[～](url)
    - 繰り返し使いたいとき：[～]: url
  - 改行：スペース2個文末に入れる
  - 段落：改行2回押すことで実現することが可能
  - 引用(多重引用可能)：> ～
  - コード機能
    - インライン：`～`
    - ブロック(言語を指定することでシンタックスハイライト可能)：```言語 ～ ```
  - 画像の挿入：![](画像url)
  - 文字の装飾(斜体＋太字：***～***)(*は_でも可能)
    - 斜体：*～*
    - 太字：**～**
...
- vscode(ショートカットキーボード)
  ...
  - 端に移動：Cmd+カーソル
  - 単語移動：Option+左右
  - 範囲選択×端に移動：Cmd+Shift+カーソル
  - 範囲選択×単語移動：Option+Shift+左右
  - やり直し：Cmd+Shift+Z
  - 検索：Cmd+F
  - 横断検索：Cmd+Shift+F
  - 定義元を参照：F12
  - ピークで定義元を参照：Option+F12
  - リファレンスを参照：Shift+F12
  - ファイル検索：Cmd+P
  - タブを閉じる：Cmd+W
  --------------------------------------
  - ファイル移動：Cmd+Option+左右
  - 新しいウィンドウ：Cmd+Shift+N
  - フォルダを閉じる：Cmd+K→F
  - 別名で保存：Cmd+Shift+S
  - 直前に閉じたタブを復元：Cmd+Shift+T
  - シンボル名を変更(ファイルをまたいで)：F2
  - シンボル名を横断検索：Cmd+T
  - ファイル内のシンボルへ移動：Cmd+Shift+O
  --------------------------------------
  - 問題タブをトグル：Cmd+Shift+M
  - カッコの始まり・終わりに移動：Cmd+Shift+＼
  - 全ての検索一致を同時選択：Cmd+F2
  - トグルブロックコメント：Option+Shift+A
  - ファイル内の文字列置換：Cmd+Option+F
  - 編集閲覧履歴：Cmd+Tab
  - 直前の編集ファイルを開く：Cmd+_, Cmd+-
  - ファイルを開く：Cmd+O
  - ズームイン：Cmd+Shift+-
  - ズームアウト：Cmd+-
  - コードのフォーマット：Option+Shift+F
  --------------------------------------
  - 行削除：Cmd+Shift+K
  - マルチカーソル：Option+Cmd+上下
  - インデント：Cmd+]
  - アウトデント：Cmd+[
  - 下に一行インサート：Cmd+Enter
  - 上に一行インサート：Cmd+Shift+Enter
  - 指定行へ移動：Cmd+G
  - サイドバーのトグル：Cmd+B
  - 次の問題個所へ移動：F8
  --------------------------------------
  - 現在の行を選択：Cmd+L
  - 開いているタブ以外を閉じる：Cmd+Option+T
  - カーソル操作の元に戻す：Cmd+U
  - すべてのタブを保存；Cmd+Option+S
  - 検索時の条件をコマンドで操作する方法
    - 小文字・大文字切り替え：Cmd+Option+C
    - ワード検索：Cmd+Option+W
    - 正規表現：Cmd+Option+R
  - 開いているファイルのパスをコピー：Cmd+K→P
  - 開いているファイルをFinderで表示：Cmd+K→R
  - 開いているファイルを新しいウィンドウで表示：Cmd+K→O
...
- [Tailwind CSS](https://tailwindcss.com/)
  - インストール方法
    - PostCSS pluginとして使う方法
    - Tailwind CLIを用いる方法
      - 用意されているCLI(コマンド)を用いる
    - CDN経由で用いる方法
      - 使用していないクラスも含まれるため、パフォーマンスが落ちる
  - 特徴
